...
candidate_parameters = {
    "SGDClassifier":            {"max_iter": [10, 100], "tol" : [1e-3]}, 
...
for this_model in candidate_models:
    if this_model.__name__ in candidate_parameters:
        ...
        for i, parameter in enumerate(parameters):
            ...
            brains.append( (f"{this_model.__name__}__{i}", brain) )
# Combine those models together
JarvisBigBrain = VotingClassifier( estimators=brains, voting="soft" )

# Prepare models for testing
testing_brains = brains.copy()
testing_brains.append(("JarvisBigBrain", JarvisBigBrain))
...
for i in range(len(names)):
    print(f"[{timers[i]:.2f} s] {names[i]}: {cv_score[i]:.4f}")